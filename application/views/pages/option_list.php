<div class="container mt-4">
    ...
    <div class="row">
        <!-- First column -->
        <div class="col-md-6">
            <form id="provinceForm" action="<?= base_url('Admin/add_option'); ?>">
                <div class="form-group">
                    <label for="typeOfCustomer"><i class="fas fa-user-tag"></i> Provinces</label>
            ...
            <form id="placeForm" action="<?= base_url('Admin/add_option'); ?>">
                <div class="form-group">
                    <label for="typeOfService"><i class="fas fa-concierge-bell"></i> Places</label>
        ...
        <!-- Second column -->
        <div class="col-md-6">
            <form id="countryForm" action="<?= base_url('Admin/add_option'); ?>">
                <div class="form-group">
                    <label for="paymentFor"><i class="fas fa-money-bill-wave"></i> Country</label>
            ...
            <form id="budgetForm" action="<?= base_url('Admin/add_option'); ?>">
                <div class="form-group">
                    <label for="modeOfPayment"><i class="fas fa-credit-card"></i> Budget</label>
...
<?php foreach (['province', 'place', 'country', 'budget'] as $type): ?>
...
<script>
            ...
            success: function(response) {
                ...
                if (response.length > 0) {
                    $.each(response, function(index, option) {
                        optionsList.append("<li class='list-group-item d-flex justify-content-between align-items-center'>" + option.value + "<button class='btn btn-danger btn-sm' onclick='prepareDelete(" + option.id + ", \"" + optionType + "\")'>Delete</button></li>");
                    });
                } else {
    ...
    $(document).ready(function() {
        $("#viewCustomerOptionsBtn").click(function() {
            loadOptions('province');
            $("#provinceOptionsModal").modal('show');
        });

        $("#viewServiceOptionsBtn").click(function() {
            loadOptions('place');
            $("#placeOptionsModal").modal('show');
        });

        $("#viewPaymentOptionsBtn").click(function() {
            loadOptions('country');
            $("#countryOptionsModal").modal('show');
        });

        $("#viewPaymentModeOptionsBtn").click(function() {
            loadOptions('budget');
            $("#budgetOptionsModal").modal('show');
        });

        $("#provinceForm, #placeForm, #countryForm, #budgetModeForm").on('submit', function(e) {
            e.preventDefault();
            var form = $(this);
            $.ajax({
                type: "POST",
    ...
    function prepareDelete(id, optionType) {
        u_id = id;
        u_option = optionType;
